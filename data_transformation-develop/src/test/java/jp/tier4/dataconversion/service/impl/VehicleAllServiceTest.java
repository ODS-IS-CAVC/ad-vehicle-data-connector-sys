...
public class VehicleAllServiceTest {

    @Autowired
    private VehicleAllService service;
    ...
    void test_service() throws Exception {
        ...
        // 検証用変数
        RetrieveAllVehicles expected = new RetrieveAllVehicles();
        expected.setTotal(0);
        ...
        vehicle2.setVehicleName("vehicle002");
        ...
        expected.setVehicles(vehicles);

        // テスト対象実行
        RetrieveAllVehicles result = service.service(null);

        // 検証
        assertThat(result.getTotal()).isEqualTo(expected.getTotal());
        vehicleAssert(result.getVehicles().get(0), expected.getVehicles().get(0));
        vehicleAssert(result.getVehicles().get(1), expected.getVehicles().get(1));
    }
